...
class reg(object):
    reg = {'reg_page': 'https://passport.58.com/reg/', 'post_page': 'https://passport.58.com/save/',
           'check_page': 'https://passport.58.com/regok?regok=1'}
    proxy = {}
    ...
    def reg_load(self, username, password, email):
        ...
        if str == '{}':
            ...
            ip = self.getproxy()
            self.t = Api_connect(ip)
            str            print uinfo, u',注册成功!'= self.t.urlopen(url=self.reg['reg_page'], savecookie=True)
        form = self.t.getForm(str)
        if form == None:
            print u'表单错误，太频繁胃'
            return
        postdata = {'nickName': '%s' % username, 'txtemail': '%s' % email, 'password': '%s' % password,
                    'cpassword': '%s' % password, 'cd': self.t.getFormValue('cd', form),
                    'ptk': self.t.getFormValue('ptk', form)}
